  ...
  return (
    ...
    initial={{opacity:0}} whileInView={{opacity: 1}} transition={{duration: 1}}>
      ...
      className='text-center max-w-2xl mx-auto mt-5 mb-12 font-Ovo'>
        I design user-friendly interfaces, build fullstack apps, analyze data, and develop machine learning models to create impactful digital products.
      </motion.p>

      <motion.div initial={{opacity:0}} whileInView={{opacity: 1}} transition={{duration: 0.6, delay: 0.9}}
      className='grid grid-cols-auto gap-6 my-10'>
        {serviceData.map(({icon, title, description, link}, index)=>(
          <motion.div key={index} className='border border-gray-400 rounded-lg px-8 py-12 hover:shadow-black cursor-pointer hover:bg-lightHover 
          hover:-translate-y-1 duration-500 dark:hover:bg-darkHover dark:hover:shadow-white text-justify'
